...
try:
    ...
    def marginandstuff(data1, data1market, ax, fig):
        ...
        ax.set_ylim( min(data1.iloc[:,0].min(), data1market.iloc[:,0].min()) - (data1.iloc[:,0].mean()/100), max(data1.iloc[:,0].max(), data1market.iloc[:,0].max()) + data1.iloc[:,0].mean()/100)
