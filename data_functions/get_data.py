    ...
    try:
        ...
        if(period == '0'):
            ...
            if interval in ('1m', '2m,' '5m', '15m', '30m', '1h'):
                ...
                if( os.path.isfile( r".\database\%s-%s-%s-%s.csv" %(symbol, period, (str)(datetime.datetime.today().date()), interval ) ) ):
                    data1 = pd.read_csv( r".\database\%s-%s-%s-%s.csv" %(symbol, period, (str)(datetime.datetime.today().date()), interval ), index_col = [0] )
