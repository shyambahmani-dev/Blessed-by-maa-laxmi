...
# most optimised value at intervalTested = 1h, buyFast = 3, buyFast = 10, sellFast = 10, sellSlow = 25
...
#"""
if( os.path.isfile( r".\database\%s-%s-%s-%s.csv" %(tickerName, periodTested, (str)(datetime.datetime.today().date()), intervalTested ) ) ):
    data1 = pd.read_csv( r".\database\%s-%s-%s-%s.csv" %(tickerName, periodTested, (str)(datetime.datetime.today().date()), intervalTested ), index_col = [0] )
    data1.index = pd.to_datetime(data1.index)
else:
    data1 = getData.tickerData(symbol= tickerName, period= periodTested, interval= intervalTested)
    data1.to_csv( r".\database\%s-%s-%s-%s.csv" %(tickerName, periodTested, (str)(datetime.datetime.today().date()), intervalTested  ) )
#"""

#data1 = getData.tickerData(symbol= tickerName, period = periodTested, intervalTested= intervalTested)
...
data1["Typical"] = (data1["High"] + data1["Low"] + data1["Close"])/3.0
...
for ind in data1.index:
    ...
    if(not pd.isna(data1EMA["%s" %(max(buySlow, sellSlow))].loc[ind])):
        ...
        if( data1EMA["%s" %(buyFast)].loc[ind] > data1EMA["%s" %(buySlow)].loc[ind]):
        ...
        elif( data1EMA["%s" %(sellFast)].loc[ind] < data1EMA["%s" %(sellSlow)].loc[ind]):
...
analysis = pa.analytics(data1, tickerName, periodTested, intervalTested, portfolio, marketPortfolio, daysBought, daysSold)
