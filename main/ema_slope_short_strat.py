...
strat_name = 'EMA Slope Strategy - Short Only'

# most optimised value at interval = 1h, signalBuy = 15, signalSell = 75
...
for ind in data1.index:
    ...
    if(not pd.isna(data1EMA["%s" %(max(signalBuy, signalSell))].loc[ind])):
        ...
        if( data1EMAslope["%s" %(signalSell)].loc[ind] < sellBelow and assetNum == 0):
        ...
        elif( data1EMAslope["%s" %(signalBuy)].loc[ind] > buyAbove):
