...
class run_strat(object):
    ...
    def run(self, longSignalBuy, longSignalSell, shortSignalBuy, shortSignalSell):
        ...
        for ind in self.data1.index:
            ...
            if(not pd.isna(self.data1EMA["%s" %(max(longSignalBuy, longSignalSell, shortSignalBuy, shortSignalSell))].loc[ind])):
                ...
                if( self.data1EMAslope["%s" %(longSignalBuy)].loc[ind] > 0):
                    ...
                    longCurrCash -= (numCanBuy*buyPrice) - min(30, (numCanBuy*(buyPrice)*self.feesFactor))
                    ...
                    longCurrCash += (numCanSell*(sellPrice)) - min(30, (numCanSell*(sellPrice))*self.feesFactor)
                ...
                if( self.data1EMAslope["%s" %(shortSignalSell)].loc[ind] < 0 and shortAssetNum == 0):
                    
                    numCanBuy = (buyRatioCash*shortCurrCash)/( buyPrice )
                    shortCurrCash += (numCanBuy*buyPrice) - min(30, (numCanBuy*(buyPrice)*self.feesFactor))
                ...
                elif( self.data1EMAslope["%s" %(shortSignalBuy)].loc[ind] > 0):

                    numCanSell = (-1*shortAssetNum)*(sellRatioPort)
                    shortCurrCash -= (numCanSell*(sellPrice)) - min(30, (numCanSell*(sellPrice))*self.feesFactor)
                    shortAssetNum = 0
...
for longSignalBuy in emaList:

    for longSignalSell in emaList:

        for shortSignalBuy in emaList:

            for shortSignalSell in emaList:

                print("Running strat for %s-%s-%s-%s" %(longSignalBuy, longSignalSell, shortSignalBuy, shortSignalSell))
                            
                stratRes = strat_object.run(longSignalBuy, longSignalSell, shortSignalBuy, shortSignalSell)
                            
                CAGRExcessRes.append( [stratRes.CAGRExcess , "%s-%s-%s-%s" %(longSignalBuy, longSignalSell, shortSignalBuy, shortSignalSell)] )
                AUCRatioRes.append( [stratRes.AUCRatio , "%s-%s-%s-%s" %(longSignalBuy, longSignalSell, shortSignalBuy, shortSignalSell)] )
                maxUpRes.append( [stratRes.maxUp , "%s-%s-%s-%s" %(longSignalBuy, longSignalSell, shortSignalBuy, shortSignalSell)] )
                maxDownRes.append( [stratRes.maxDown , "%s-%s-%s-%s" %(longSignalBuy, longSignalSell, shortSignalBuy, shortSignalSell)] )
                averageUpRes.append( [stratRes.averageUp , "%s-%s-%s-%s" %(longSignalBuy, longSignalSell, shortSignalBuy, shortSignalSell)] )
