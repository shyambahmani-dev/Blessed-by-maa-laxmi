...
# currently running SBIN.NS, TATASTEEL.NS, INFY.NS


tickerName = "INFY.NS"
periodTested = "10y"
strat_name = "ema_slope"
intervalTested = '1h'
    ...
    def run(self, signalBuy, signalSell, buyAbove, sellBelow):
        ...
        for ind in self.data1.index:
            ...
            if(not pd.isna(self.data1EMA["%s" %(max(signalBuy, signalSell))].loc[ind])):
                ...
                if( self.data1EMAslope["%s" %(signalBuy)].loc[ind] > buyAbove):
                ...
                elif( self.data1EMAslope["%s" %(signalSell)].loc[ind] < sellBelow):
